...
| ZTP-012 | 2026-09-21 (l9mxc) | `hs-quay` stays Progressing (`UpgradePending`, InstallPlan missing); MCE CSV `Pending`/`InstallCheckFailed` | Wipe deleted InstallPlans/CSVs while Subscription status still referenced them | Wipe deletes Subscription+IP+CSV+OG in quay/mce/ocm and waits until gone | 1 (fixing) |
